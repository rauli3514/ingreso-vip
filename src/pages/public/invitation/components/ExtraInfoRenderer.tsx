...
export default function ExtraInfoRenderer({ title, subtitle, blocks, onSuggestSong, themeColor, componentsConfig }: Props) {
    ...
    // Si hay un subtitle pero NO hay blocks (modo simplificado del AdvancedEditor)
    const isSimpleMode = !blocks || Object.keys(blocks).length === 0;
    ...
    return (
        <section className="py-24 relative overflow-hidden bg-[#F4F5F1]">
            <div className="container mx-auto px-4 text-center relative z-10">
                {title && <h2 className="text-3xl md:text-5xl font-serif mb-8" style={{ color: mainColor }}>{title}</h2>}
                
                {isSimpleMode ? (
                    // Modo Simplificado (Solo texto)
                    <div className="max-w-3xl mx-auto">
                        <div className={cardClass}>
                            <div className="mb-6 p-4 text-stone-700 group-hover:scale-110 transition-transform duration-300">
                                <ClipboardList size={48} strokeWidth={1} />
                            </div>
                            <div className="text-base md:text-lg text-stone-500 font-light leading-relaxed whitespace-pre-line flex-1 text-left w-full">
                                {subtitle || 'Hagamos juntos una fiesta épica. Aquí algunos detalles a tener en cuenta.'}
                            </div>
                        </div>
                    </div>
                ) : (
                    // Modo VIP (Bloques separados)
                    <>
