                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-800/50">
                                        ...
                                        <p className="text-lg font-bold text-slate-200">
                                            {((event as any).guests && (event as any).guests[0] && (event as any).guests[0].count > 0) ? (event as any).guests[0].count : ((event as any).planner_data?.guests?.length || event.guest_count_total)}
                                        </p>
                                    </div>
                                    <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-800/50">
                                        <p className="text-xs text-slate-500 font-medium mb-1 flex items-center gap-1.5"><MapPin size={14} /> Mesas</p>
                                        <p className="text-lg font-bold text-slate-200">
                                            {((event as any).planner_data?.tables?.length || event.table_count)}
                                        </p>
                                    </div>
                                </div>
                            </div>
